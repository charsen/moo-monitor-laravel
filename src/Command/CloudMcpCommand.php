...
class CloudMcpCommand extends Command
{
    ...
    private function serverInstructions(): string
    {
        return implode("\n", [
            ...
            '',
            '另暴露团队「待办」(测试/产品经 Chrome 扩展提报的 bug 单,带页面 URL / 失败请求 / JS 错误上下文)。工作流:',
    ...
    private function toolDefinitions(): array
    {
        return [
            ...
            [
                ...
                'description' => '列出本项目在云端「可处理」的待办(测试/产品提报的 bug 单,默认 open + in_progress,最新优先)。返回每条的 id、标题、优先级、状态、页面 URL。',
                'inputSchema' => [
                    'type'       => 'object',
                    'properties' => [
                        'limit'  => ['type' => 'integer', 'description' => '返回条数,1–50,默认 20', 'minimum' => 1, 'maximum' => 50],
